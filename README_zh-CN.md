...
- ✅ 用不同语言阅读: [🇨🇳](./README_zh.md) 或者帮忙翻译其他语言: [ [🇪🇸](./README-es.md) , [🇩🇪](./README-de.md) , ... ]
- 🙏 欢迎贡献
...
❌ **除此以外：**如果不能在支持用户方面提供可操作的帮助，将会因为缺乏操作CLI的能力而导致受挫。
...
❌ **除此以外:** 要求您的用户通过多次调用 CLI 重复提供相同的信息，将会使您的用户感到烦恼。
...
❌ **除此以外:** 苍白的程序输出中可能容易丢失信息，尤其是当输出文本繁重的时候。
...
❌ **除此以外:** 当数据是以封闭选项(即下拉菜单)的形式出现时，作为输入的文本提示可能对用户来说很麻烦。
...
❌ **除此以外:** 避免像`git.io/abc`这样需要用户手动复制和粘贴的中断且非交互式的链接。
...
❌ **除此以外:** 如果可以可靠的方式自动检测命令行参数，并且调用的操作不显式要求用户交互(例如确认删除)，则不要强制用户交互。
...
❌ **除此之外:** 您的程序不能与其他程序很好地配合，并会引入意外的行为。
...
- 6.1 [信息性错误](#61-%E4%BF%A1%E6%81%AF%E6%80%A7%E9%94%99%E8%AF%AF)
- 6.1 [可行性错误](#61-%E5%8F%AF%E8%A1%8C%E6%80%A7%E9%94%99%E8%AF%AF)
- 6.3 [提供调试模式](#63-%E6%8F%90%E4%BE%9B%E8%B0%83%E8%AF%95%E6%A8%A1%E5%BC%8F)
- 6.4 [正确使用退出代码](#64-%E6%AD%A3%E7%A1%AE%E4%BD%BF%E7%94%A8%E9%80%80%E5%87%BA%E4%BB%A3%E7%A0%81)
...
❌ **Otherwise:** 不要跳过调试功能。 要从用户那里收集反馈，并找出错误的原因将变得更加困难。
...
[![All Contributors](https://img.shields.io/badge/all_contributors-4-orange.svg?style=flat-square)](#%E8%B4%A1%E7%8C%AE%E8%80%85)
